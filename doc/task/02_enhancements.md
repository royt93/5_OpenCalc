...
- **E-INFRA-6 — Chuyển `UIUtils.setupEdgeToEdge1/2` vào `BaseActivity`** — hiện mỗi Activity con phải tự nhớ gọi cặp hàm này trong `onCreate()` (xem [F-UI-12](01_fixes.md)); đã quên sót 2 lần liên tiếp khi thêm `BillSplitterActivity`/`BaseConverterActivity` cùng đợt, khiến nội dung bị status bar đè. Override `setContentView` trong `BaseActivity` để tự động gọi `setupEdgeToEdge1` trước khi inflate + `setupEdgeToEdge2` sau khi set, mọi activity con kế thừa sẽ tự động đúng mà không cần nhớ copy-paste. P1 · S — rủi ro thấp nếu làm cẩn thận (activity cũ đã tự gọi 2 hàm này sẽ bị gọi TRÙNG LẶP, cần rà soát bỏ lời gọi thủ công ở từng activity cũ khi chuyển vào BaseActivity).
